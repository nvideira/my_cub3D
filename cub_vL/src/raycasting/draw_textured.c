...
/*   Updated: 2023/07/17 02:13:44 by nvideira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_tex(int side)
{
	...
	vars()->play->step_x = 1.0 * TEX_RES / vars()->graph->line_height;
	tex_pos = (vars()->graph->draw_start - WIN_HEI / 2 + vars()->graph->line_height / 2) * vars()->play->step_x;
	while (y < vars()->graph->draw_end)
	{
		tex_y = (int)tex_pos & (TEX_RES - 1);
		tex_pos += vars()->play->step_x;
